...
2. **権限をリセットして再設定**（推奨）
   ...
   権限が正しく反映されない場合、以下のコマンドで権限をリセットしてから再設定してください：
   ...
   その後：
   - システム環境設定 > プライバシーとセキュリティ > アクセシビリティ
   - Honnyakuを探してチェックを入れる（リストにない場合は「+」ボタンで追加）
   - **アプリを完全に再起動**（Cmd+Qで終了してから再度起動）

3. **アプリを完全に再起動**
   - Honnyakuを完全に終了（Cmd+Q）
   - ターミナルでプロセスが完全に終了しているか確認：
     ```bash
     ps aux | grep honnyaku | grep -v grep
     ```
     何も表示されなければOK
   - 再度起動
