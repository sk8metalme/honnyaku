...
### 改善
- **モデル表示の改善**
  - Claude CLIを使用している場合、「Claude CLI」と明示的に表示されるように改善
  - Ollamaを使用している場合は、従来通りモデル名を表示
  - どの翻訳プロバイダーを使用しているか一目で分かるようになり、UXが向上

## [0.5.4] - 2025-12-12

### 追加
- **アクセシビリティ権限デバッグ機能**
  - デバッグ情報表示ボタンを追加（Bundle ID、Bundle Path、実行ファイルパスを表示）
  - 権限チェックの詳細なログ出力を追加
  - システム環境設定での権限付与後の詳しい手順を表示

### 改善
- **ユーザーガイダンスの強化**
  - アプリの完全な再起動が必要であることを明示的に通知
  - トラブルシューティングが容易になるデバッグ情報の提供
  - 権限問題の解決手順をより詳しく説明
...
### 修正
- **アクセシビリティ権限検出**
  - macOSアクセシビリティ権限が正しく検出されない問題を修正
  - 再チェックボタンが「確認中」のままになる問題を修正
  - `AXIsProcessTrusted()` ネイティブAPIを使用した権限チェックを実装
  - プラグインの代わりにネイティブAPIを直接使用して信頼性を向上
